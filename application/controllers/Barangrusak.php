...
class Barangrusak extends CI_Controller
{
    ...
    {
        $this->form_validation->set_rules('tanggal_rusak', 'Tanggal Rusak', 'required|trim');
        ...
        $this->form_validation->set_rules(
            ...
            [
                'less_than' => "Jumlah Rusak tidak boleh lebih dari {$stok}"
            ]
        );
    }
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Aset Dipinjam";
            $data['barang'] = $this->admin->get('barang', null, ['stok >' => 0]);

            // Mendapatkan dan men-generate kode transaksi barang rusak
            $kode = 'T-BR-' . date('ymd');
            $kode_terakhir = $this->admin->getMax('barang_rusak', 'id_barang_rusak', $kode);
            $kode_tambah = substr($kode_terakhir, -5, 5);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 5, '0', STR_PAD_LEFT);
            $data['id_barang_rusak'] = $kode . $number;

            $this->template->load('templates/dashboard', 'barang_rusak/add', $data);
        } else {
            $input = $this->input->post(null, true);
            $insert = $this->admin->insert('barang_rusak', $input);
    ...
    public function delete($getId)
    {
        ...
        if ($this->admin->delete('barang_rusak', 'id_barang_rusak', $id)) {
