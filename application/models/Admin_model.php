...
class Admin_model extends CI_Model
{
    ...
    public function getBarangKembali($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('*');
        $this->db->join('user u', 'bb.user_id = u.id_user');
        $this->db->join('supplier sp', 'bb.supplier_id = sp.id_supplier');
        $this->db->join('barang b', 'bb.barang_id = b.id_barang');
        $this->db->join('karyawan k', 'bb.karyawan_id = k.id_karyawan');
        $this->db->join('satuan s', 'b.satuan_id = s.id_satuan');
        if ($limit != null) {
            $this->db->limit($limit);
        }
        ...
        $this->db->order_by('id_barang_kembali', 'DESC');
        return $this->db->get('barang_kembali bb')->result_array();
    }
    ...
    public function getBarangMasuk($limit = null, $id_barang = null, $range = null)
    {
        ...
        // $this->db->join('karyawan k', 'bm.karyawan_id = k.id_karyawan');
        $this->db->join('satuan s', 'b.satuan_id = s.id_satuan');
        if ($limit != null) {
            $this->db->limit($limit);
        }
    ...
    public function getBarangKeluar($limit = null, $id_barang = null, $range = null)
    {
        ...
        $this->db->join('karyawan k', 'bk.karyawan_id = k.id_karyawan');
        $this->db->join('satuan s', 'b.satuan_id = s.id_satuan');
        if ($limit != null) {
            $this->db->limit($limit);
        }
